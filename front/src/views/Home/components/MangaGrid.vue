...
import { buildSearchIndex, buildSearchKeywordGroups } from "@/utils";
...
const indexedMangas = computed(() => {
    return mangas.value.map((manga) => ({
        manga,
        searchIndex: buildSearchIndex(`${manga.name} ${manga.path}`),
    }));
});

const keywordGroups = computed(() => {
    return buildSearchKeywordGroups(searchQuery.value);
});
...
const filteredMangas = computed(() => {
    ...
    return indexedMangas.value
        .filter(({ searchIndex }) => {
            return keywordGroups.value.every((group) => {
                return group.some((keyword) => searchIndex.includes(keyword));
            });
        })
        .map(({ manga }) => manga);
});
